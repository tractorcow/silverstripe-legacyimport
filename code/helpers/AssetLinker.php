...
class AssetLinker extends LegacyHelper
{
    ...
    {
        if (!defined('SS_REMOTE_SITE')) {
            throw new InvalidArgumentException('SS_REMOTE_SITE not defined');
        }
    ...
    protected function findRemoteFile($where = array())
    {
        ...
        if ($remoteFile) {
            return new ArrayData($remoteFile);
        }
    }
    ...
    protected function importFile($filename)
    {
        ...
        if (!$localFile) {
            $localFile = $this->makeLocalFile($filename);
        }
